...
namespace Slk.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = _userService.SignIn(model.Email, model.Password);
            switch (result.Success)
            {
                ...
                default:
                    ...
                    */
                case true:
                    signInHelper.SignIn((User)result.Data["user"], false, true);
                    //FormsAuthentication.SetAuthCookie(model.Email, true);
                    return RedirectToLocal(returnUrl ?? "/");
                    //return RedirectToLocal("/");
                default:
                    ModelState.AddModelError("", result.ErrorsCombined);
                    return View(model);
            }
        }
        ...
        */
        ...
        public ActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //var user = new User { Name = Email = model.Email };

                //var result = await UserManager.CreateAsync(user, model.Password);
                var result = _userService.Create((User)model, model.Password);
                if (result.Success)
                {
                    //await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                    var user = (User)result.Data["user"];
                    signInHelper.SignIn(user, false, true);
